      ...
      <Tabs v-model="activeTab" class="w-full">
        ...
        <TabsContent v-if="canImport" value="import" class="space-y-4 mt-4">
          ...
          <template v-else>
            ...
            <!-- Import Result -->
            <div v-if="importResult" class="rounded-md p-3 space-y-2" :class="importResult.errors > 0 ? 'bg-amber-500/10 border border-amber-500/20' : 'bg-primary/10 border border-primary/25'">
              <div class="flex items-center gap-2">
                <Check v-if="importResult.errors === 0" class="h-4 w-4 text-primary" />
