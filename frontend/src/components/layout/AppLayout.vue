...
<template>
  <div class="flex h-screen min-h-0 bg-background">
    <!-- Skip link for accessibility -->
    <a href="#main-content" class="skip-link">{{ $t('nav.skipToMain') }}</a>

    <!-- Mobile header -->
    <header class="fixed top-0 left-0 right-0 z-50 flex min-h-12 items-center justify-between border-b border-border bg-background/95 backdrop-blur-sm px-3 pt-[env(safe-area-inset-top)] md:hidden">
      <RouterLink to="/" class="flex items-center gap-2">
        <div class="h-7 w-7 rounded-lg bg-gradient-to-br from-primary to-blue-700 flex items-center justify-center shadow-md shadow-primary/20">
      ...
      <Button
        variant="ghost"
        size="icon"
        class="h-10 w-10 min-h-[44px] min-w-[44px] text-muted-foreground hover:text-foreground hover:bg-muted"
        ...
        <nav class="px-2" role="menubar">
          <template v-for="(section, sIdx) in mainSections" :key="section.label">
            ...
            <div
              ...
              :class="['px-2.5 pt-4 pb-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground', sIdx === 0 && 'pt-1']"
            >
              {{ $t(section.label) }}
            </div>
    ...
    <!-- Main content -->
    <main id="main-content" class="flex-1 min-h-0 overflow-hidden pt-[calc(3rem+env(safe-area-inset-top))] md:pt-0 bg-background" role="main">
